...
$schedQuery = mysqli_query($conn, "SELECT * FROM `schedule` WHERE section = '$section'");
...
foreach ($time_slots as $index => $time_slot) {
    ...
    foreach ($days as $day) {
        ...
        ?> <?php if (!empty($sched[$slot_name])): ?>
         <input readonly type="text" class="subject" value="<?= htmlspecialchars($sched[$slot_name]) ?>">
        <?php else: ?>
            <input readonly type="text" class="select" value="TBA">
        <?php endif; ?>
        <?php
    }
}
